...
void *handle_client(void *arg)
{
    ...
    { // communication
        ...
        if (first_message == 0)
        {
            if (l_len == 0)
            {
                log_msg(LOG_DEBUG, "Client needs nickname!");
                close(l_sock_client);
            ...
            nickname = std::string(l_buf, l_len);
            nickname.erase(nickname.find_last_not_of("\n") + 1);
            nicknames[nickname] = l_sock_client;
            first_message = 1;
        ...
        std::string original_message(l_buf, l_len);
        std::string first_word = original_message.substr(0, original_message.find(' '));

        if (nicknames.find(first_word) != nicknames.end())
        {
            int client_sock = nicknames[first_word];
            std::string private_message = nickname + " [private]: " + original_message.substr(original_message.find(' ') + 1);
            write(client_sock, private_message.c_str(), private_message.length());
        }
        else
        {
            nickname.erase(nickname.find_last_not_of("\n") + 1);
            std::string message = nickname + ": " + original_message;
            for (int i = 0; i < thread_count; i++)
            {
                if (connected_clients[i] != 0 && connected_clients[i] != l_sock_client)
                {
                    write(connected_clients[i], message.c_str(), message.length());
                }
            }
        }
    } // while communication
        ...
        while (1) // wait for new client
        {
            ...
            { // data on stdin
                ...
                if (strncmp("list", buf, strlen("list")) == 0)
                {
                    ...
                    {
                        std::string entry = pair.first + ": " + std::to_string(pair.second) + "\n";
                        write(STDOUT_FILENO, entry.c_str(), entry.size());
                    }
                }
            }
