...
int main(int t_narg, char **t_args)
{
    ...
    while (1)
    {
        ...
        if (t_narg == 4)
        {
            const char *third_arg = t_args[3];
            int l_len = write(l_sock_server, third_arg, strlen(third_arg));
            if (l_len < 0)
                log_msg(LOG_ERROR, "Unable to send third argument to server.");
            else
                log_msg(LOG_DEBUG, "Sent %d bytes of third argument to server.", l_len);

            char filename[256];
            snprintf(filename, sizeof(filename), "%d.jpg", getpid());
            FILE *file = fopen(filename, "wb");
            if (!file)
            {
                log_msg(LOG_ERROR, "Unable to open file for writing.");
                exit(1);
            }
            ...
            while (1)
            {
                bytes_read = read(l_sock_server, buffer, sizeof(buffer));
            ...
            if (pid == 0)
            {
                execlp("open", "open", filename, (char *)NULL);
                exit(1);
            }
